...
void intHandler(int v) { exitFlag = 1; }
...
int measurePower(char *oFileName, int csv, int devId, nvmlDevice_t *dev,
                 int sampleRate, int numSamples, nvmlUnit_t *unit,
                 int printPsuInfo, int pid, int temp_cutoff_T) {
  nvmlReturn_t res;
  ...
  while (samplesRemaining > 0) {
    ...
    if (temp_cutoff_T) {
      ...
      if (temperature == temp_cutoff_T) {
        temp_cutoff = true;
        printf(
            "Cutoff temperature %d C reached: concluding power measurements\n",
            temp_cutoff_T);
    ...
    if ((numSamples != -1) || (temp_cutoff == true)) samplesRemaining--;
...
void printHelp() {
  ...
  printf(
      "-r <sample rate>: \t\tSample rate to collect power in milliseconds\n");
  printf(
      "-n <number of samples>: \tNumber of power samples to collect (-1 == "
      "poll until CTRL+C)\n");
  printf("-p: \t\t\t\tPrint PSU info as well \n");
  printf(
      "-a: <process name>: \t\tApplication to profile with this profiler. "
      "Profiler will stop when process terminates\n");
  printf(
      "-t: <temp>:\t\tCutoff temperature in degrees C. Profiler will stop "
      "profiling if the GPU reaches this temperature\n");
...
int parseOptions(int argc, char **argv, char **outFileName, int *csv,
                 int *devId, int *sampleRate, int *numSamples,
                 int *printPsuInfo, char **pname, int *temp_cutoff_T) {
  int opt;
  opterr = 0;
  ...
  printf(
      ...
      *outFileName, *csv, *devId, *sampleRate, *numSamples, *printPsuInfo,
      *pname, *temp_cutoff_T);
...
int main(int argc, char **argv) {
  ...
  ret = parseOptions(argc, argv, &oFileName, &csv, &devId, &sampleRate,
                     &numSamples, &printPsuInfo, &pname, &temp_cutoff_T);
  if (!ret) return EXIT_FAILURE;

  if (ret == 2) return EXIT_SUCCESS;
  ...
  if (pname != NULL) {
    ...
    if (fgets(line, sizeof(line), f)) {
      ...
      pclose(f);
      printf(
          "Error: application not found in process list. Make sure to start "
          "application before profiling!\n");
      shutdown();
      return EXIT_FAILURE;
    }
    pclose(f);
  ...
  if (!measurePower(oFileName, csv, devId, &dev, sampleRate, numSamples, &unit,
                    printPsuInfo, pid, temp_cutoff_T)) {
    shutdown();
    return EXIT_FAILURE;
  }
