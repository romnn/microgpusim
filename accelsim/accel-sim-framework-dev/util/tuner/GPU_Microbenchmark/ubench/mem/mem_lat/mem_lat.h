...
#define THREADS_NUM \
  4  // HERE, we launch four threads, to ensure that one request is equal to
     // DRAM trascation, 4 thread * 8 bytes = 32 bytes (= min DRAM trascation)
...
float mem_lat() {
  ...
  unsigned MEM_ARRAY_SIZE =
      ...
      2;  // pointer-chasing array size in 64-bit. total array size is 7 MB
          // which larger than L2 cache size (6 MB in Volta) to avoid l2 cache
          // resident from the copy engine
