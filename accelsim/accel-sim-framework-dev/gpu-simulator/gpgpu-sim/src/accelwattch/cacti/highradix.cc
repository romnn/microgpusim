...
HighRadix::HighRadix(double SUB_SWITCH_SZ_, double ROWS_,
                     double FREQUENCY_,  // GHz
                     double RADIX_, double VC_COUNT_, double FLIT_SZ_,
                     double AF_,       // activity factor
                     double DIE_LEN_,  // u
                     double DIE_HT_,   // u
                     double INP_BUFF_ENT_, double ROW_BUFF_ENT_,
                     double COL_BUFF_ENT_, TechnologyParameter::DeviceType *dt)
    : SUB_SWITCH_SZ(SUB_SWITCH_SZ_),
      ROWS(ROWS_),
      ...
      DIE_LEN(DIE_LEN_),
      DIE_HT(DIE_HT_),
      INP_BUFF_ENT(INP_BUFF_ENT_),
      ROW_BUFF_ENT(ROW_BUFF_ENT_),
      COL_BUFF_ENT(COL_BUFF_ENT_),
      deviceType(dt) {
  double area_scale = 1;
  double tech_init = 90;
  if (g_ip->F_sz_nm == 65) {
...
void HighRadix::compute_power() {
  ...
  // effective ht of vertical bus (connecting cb to column buffer) in each
  // sub-switch
  double eff_ht = (ROWS * (ROWS + 1) / 2) * (DIE_HT / ROWS);
  ver_bus = new Wire(g_ip->wt, eff_ht);

  // sub switch includes row buffers, column buffers, vc/crossbar/column
  // arbitration and a 2 stage crossbar traversal
  sub_switch_power();
  ...
  power.readOp.dynamic +=
      hor_bus->power.readOp.dynamic * FLIT_SZ * SUB_SWITCH_SZ * ROWS;
  power.readOp.leakage +=
      hor_bus->power.readOp.leakage * FLIT_SZ * SUB_SWITCH_SZ * ROWS;
  power.readOp.dynamic +=
      ver_bus->power.readOp.dynamic * FLIT_SZ * COLUMNS * SUB_SWITCH_SZ;
  power.readOp.leakage +=
      ver_bus->power.readOp.leakage * FLIT_SZ * ROWS * COLUMNS * SUB_SWITCH_SZ;
  ...
  sub_sw.area.set_area(sub_sw.area.get_area() +
                       r_buff->area.get_area() * VC_COUNT * SUB_SWITCH_SZ);
  sub_sw.area.set_area(sub_sw.area.get_area() +
                       c_buff->area.get_area() * VC_COUNT * SUB_SWITCH_SZ);

  buff_tot.area.set_area(buff_tot.area.get_area() +
                         inp_buff->area.get_area() * RADIX);
  buff_tot.area.set_area(buff_tot.area.get_area() +
                         VC_COUNT * r_buff->area.get_area() * SUB_SWITCH_SZ *
                             num_sub);
  buff_tot.area.set_area(buff_tot.area.get_area() +
                         VC_COUNT * c_buff->area.get_area() * SUB_SWITCH_SZ *
                             num_sub);

  crossbar_tot.area.set_area(crossbar_tot.area.get_area() +
                             cb->area.get_area() * num_sub);
  crossbar_tot.area.set_area(crossbar_tot.area.get_area() +
                             out_cb->area.get_area() * num_sub);

  wire_tot.area.set_area(hor_bus->area.get_area() * FLIT_SZ * SUB_SWITCH_SZ *
                         ROWS);
...
void HighRadix::compute_bus_power() {
  wire_tot.power.readOp.dynamic =
      hor_bus->power.readOp.dynamic * FLIT_SZ * SUB_SWITCH_SZ * ROWS;
  wire_tot.power.readOp.leakage =
      hor_bus->power.readOp.leakage * FLIT_SZ * SUB_SWITCH_SZ * ROWS;
  wire_tot.power.readOp.dynamic +=
      ver_bus->power.readOp.dynamic * FLIT_SZ * COLUMNS * SUB_SWITCH_SZ;
  wire_tot.power.readOp.leakage +=
...
void HighRadix::sub_switch_power() {
  // each sub-switch power
  sub_sw.power.readOp.dynamic =
      sub_sw.power.readOp.dynamic +
      r_buff->power.readOp.dynamic * 2 /* one read and one write */ * VC_COUNT;
  sub_sw.power.readOp.leakage =
      sub_sw.power.readOp.leakage + r_buff->power.readOp.leakage * VC_COUNT;
  ...
  sub_sw.power.readOp.leakage =
      sub_sw.power.readOp.leakage + c_buff->power.readOp.leakage * VC_COUNT;
...
Mat *HighRadix::buffer_(double block_sz, double sz) {
  ...
  dyn_p.ram_cell_tech_type = dyn_p.num_r_subarray = (int)(sz / block_sz);
...
void HighRadix::print_buffer(Component *c) {
  ...
  cout << "\tDynamic Power - " << c->power.readOp.dynamic * 1e9 << " nJ"
       << endl;
  cout << "\tLeakage Power - " << c->power.readOp.leakage * 1e3 << " mW"
       << endl;
...
void HighRadix::print_router() {
  ...
  cout << "\n\n MCPAT_Router dynamic power (max) = "
       << power.readOp.dynamic * FREQUENCY * 1e9 << " W\n";
  cout << " MCPAT_Router dynamic power (load - " << AF
       << ") = " << power.readOp.dynamic * FREQUENCY * 1e9 * AF << " W\n";
  cout << "\n\nDetailed Stats\n";
  cout << "--------------\n";
  ...
  cout << "\t" << setiosflags(ios::fixed) << setprecision(2)
       ...
       << " %\n";
  cout << "Buffer power                    - "
       << buff_tot.power.readOp.dynamic * FREQUENCY * 1e9 << " W";
  cout << "\t" << (buff_tot.power.readOp.dynamic / power.readOp.dynamic) * 100
       << " %\n";
  cout << "Crossbar power                  - "
       << crossbar_tot.power.readOp.dynamic * FREQUENCY * 1e9 << " W";
  cout << "\t"
       << (crossbar_tot.power.readOp.dynamic / power.readOp.dynamic) * 100
       << " %\n";
  cout << "Arbiter power                   - "
       << arb_tot.power.readOp.dynamic * FREQUENCY * 1e9 << " W";
  cout << "\t" << (arb_tot.power.readOp.dynamic / power.readOp.dynamic) * 100
       << " %\n";
  cout << "Sub-switch power (dynamic)      - "
       << sub_sw.power.readOp.dynamic * num_sub * FREQUENCY * 1e9 << " W";
  cout << "\t"
       << (sub_sw.power.readOp.dynamic * num_sub / power.readOp.dynamic) * 100
       << " %\n";
  cout << "Input buffer power (dynamic)    - "
       << 2 * inp_buff->power.readOp.dynamic * RADIX * FREQUENCY * 1e9 << " W";
  cout << "\t"
       << (2 * inp_buff->power.readOp.dynamic * RADIX / power.readOp.dynamic) *
              100
       << " %\n";
  cout << "\nLeakage power\n";
  cout << "MCPAT_Router power                    - " << power.readOp.leakage
       << " W\n";
  cout << "Bus power                       - " << setprecision(4)
       << wire_tot.power.readOp.leakage << " W\n";
  cout << "Buffer power                    - " << buff_tot.power.readOp.leakage
       << " W\n";
  cout << "Crossbar power                  - "
       << crossbar_tot.power.readOp.leakage << " W\n";
  cout << "Arbiter power                   - " << arb_tot.power.readOp.leakage
       << " W\n";
  cout << "Sub-switch power                - " << sub_sw.power.readOp.leakage
       << " W" << endl;

  cout << "\n\nArea Stats\n";
  cout << "Input buffer dimension (mm x mm)- " << inp_buff->area.get_h() * 1e-3
       << " x " << inp_buff->area.get_w() * 1e-3 << endl;
  cout << "Row buffer (mm x mm)            - " << r_buff->area.w * 1e-3 << " x "
       << r_buff->area.h * 1e-3 << endl;
  cout << "Col buffer (mm x mm)            - " << c_buff->area.w * 1e-3 << " x "
       << c_buff->area.h * 1e-3 << endl;
  cout << "Crossbar area  (mm x mm)        - " << cb->area.w * 1e-3 << " x "
       << cb->area.h * 1e-3 << endl;
  //  cout << "Wire hor area  (nm x nm)        - " << hor_bus->area.w*1e3 << " x
  //  " << hor_bus->area.h*1e3 << endl; cout << "Wire ver area  (nm x nm) - " <<
  //  ver_bus->area.w*1e3 << " x " << ver_bus->area.h*1e3 << endl;
  cout << "Wire total                      - "
       << wire_tot.area.get_area() * 1e-6 << " mm2\n";
  cout << "Crossbar total                  - "
       << crossbar_tot.area.get_area() * 1e-6 << " mm2\n";
  cout << "Buff total                      - "
       << buff_tot.area.get_area() * 1e-6 << " mm2\n";
  cout << "Subswitch                       - " << sub_sw.area.get_area() * 1e-6
       << " mm2\n";
  cout << "Subswitch total                 - "
       << sub_sw.area.get_area() * num_sub * 1e-6 << " mm2\n";

  cout << "Total area                      - "
       << (wire_tot.area.get_area() + crossbar_tot.area.get_area() +
           buff_tot.area.get_area()) *
              1e-6
       << endl;
}
