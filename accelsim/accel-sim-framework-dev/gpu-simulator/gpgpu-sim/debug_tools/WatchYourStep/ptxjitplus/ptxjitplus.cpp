 ...
 * the arrays from the kernel. It allows you to watch each step you program
 * takes, kernel by kernel.
 *
 ...
 *      e)Make sure all GPGPU-Sim path variables are set (see GPGPU-Sim
 * documentation)
 * 2. Run executable (program to debug) using GPGPU-Sim
 * 3. export PTX_SIM_DEBUG=[less than 4 to not dump config files again]
 * 4-1. Run one kernel at a time: export WYS_LAUNCH_NUM=[kernel to launch] and
 * compile ptxjitplus and run ptxjitplus 4-2. Run all kernels: compile and run
 * ". launchkernels 0 [max number of kernels]" in terminal
...
void initializeData(std::vector<param> &v_params) {
  ...
  std::string path_to_search =
      std::string(wys_exec_path) + "/" + wys_exec_name + ".*.ptx";
...
int main(int argc, char **argv) {
  ...
  if (deviceProp.major < 2) {
    fprintf(stderr,
            "Compute Capability 2.0 or greater required for this sample.\n");
  ...
  int index = 0;
  for (std::vector<param>::iterator p = v_params.begin(); p != v_params.end();
       p++) {
    if (p->isPointer) {
      unsigned char **d_data =
          (unsigned char **)malloc(sizeof(unsigned char **));
      checkCudaErrors(cudaMalloc((void **)d_data, p->size));
      checkCudaErrors(cudaMemcpy((void *)*d_data, (void *)p->data, p->size,
  ...
  CUDAAPI cuLaunchKernel(hKernel, gridDim.x, gridDim.y, gridDim.z, blockDim.x,
                         blockDim.y, blockDim.z, 0, NULL, paramKernels, NULL);
  ...
  for (std::map<size_t, void *>::iterator i = m_device_data.begin();
       i != m_device_data.end(); i++) {
    ...
    checkCudaErrors(cudaMemcpy(h_data, i->second, v_params[i->first].size,
  ...
  assert(fout);
  for (std::map<size_t, unsigned char *>::iterator i = m_output_data.begin();
       i != m_output_data.end(); i++) {
    fprintf(fout, "param %zu: size = %zu, data = ", i->first,
            v_params[i->first].size);
  ...
  for (std::map<size_t, void *>::iterator i = m_cleanup.begin();
       i != m_cleanup.end(); i++) {
    if (i->second) {
      free(i->second);
      i->second = 0;
