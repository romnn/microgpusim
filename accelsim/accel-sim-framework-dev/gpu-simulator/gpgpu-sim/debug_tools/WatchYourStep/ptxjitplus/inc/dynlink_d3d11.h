...
static bool dynlinkLoadD3D11API(void) {
  ...
  if (s_hModD3D11 != NULL) {
    sFnPtr_D3D11CreateDevice =
        (LPD3D11CREATEDEVICE)GetProcAddress(s_hModD3D11, "D3D11CreateDevice");
  ...
  } else  // if absent try to take it from DirectX 2010 Feb
  {
    ...
    if (s_hModD3DX11 != NULL) {
      sFnPtr_D3DX11CompileFromMemory =
          (LPD3DX11COMPILEFROMMEMORY)GetProcAddress(s_hModD3DX11,
  ...
  if (!sFnPtr_CreateDXGIFactory) {
    ...
    if (s_hModDXGI) {
      sFnPtr_CreateDXGIFactory =
          (LPCREATEDXGIFACTORY)GetProcAddress(s_hModDXGI, "CreateDXGIFactory1");
    }
