...
inline void getCudaAttribute(T *attribute, CUdevice_attribute device_attribute,
  ...
  if (error_result != CUDA_SUCCESS) {
    printf("cuDeviceGetAttribute returned %d\n-> %s\n", (int)error_result,
           getCudaDrvErrorString(error_result));
    exit(EXIT_SUCCESS);
  }
}
#endif
...
inline int gpuGetMaxGflopsGLDeviceIdDRV() {
  ...
  if (device_count == 0) {
    fprintf(stderr,
            "gpuGetMaxGflopsGLDeviceIdDRV error: no devices supporting CUDA\n");
    exit(EXIT_FAILURE);
  }
  ...
  while (current_device < device_count) {
    ...
    checkCudaErrors(cuDeviceGetAttribute(&bTCC, CU_DEVICE_ATTRIBUTE_TCC_DRIVER,
                                         current_device));
#else
  ...
  if (devices_prohibited == device_count) {
    fprintf(stderr,
            "gpuGetMaxGflopsGLDeviceIdDRV error: all devices have compute mode "
            "prohibited.\n");
    exit(EXIT_FAILURE);
  }
