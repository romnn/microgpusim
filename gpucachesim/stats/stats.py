...
class Stats(common.Stats):
    ...
    def _compute_trace_time_sec(self):
        traces_dir = Path(self.target_config["traces_dir"])
        with open(traces_dir / "trace_time.json", "r") as f:
        ...
        #     print(k)
        #     print(df)
        # print(grouped[[c for c in PREVIEW_COLS if c in self.sim_df]].mean())
        # print(len(grouped))
        self.result_df["trace_time_sec"] = trace_exec_time_sec
    ...
    def _compute_dram_accesses(self):
        ...
        accesses = df[is_global & ~no_kernel]
        grouped = accesses.groupby(INDEX_COLS, dropna=False, sort=False)
        # grouped.apply(lambda dff: print(dff))
        self.result_df["dram_accesses"] = grouped["num_accesses"].sum()
...
class ExecDrivenStats(Stats):
    ...
    def _compute_trace_time_sec(self):
        ...
        # print(self.config)
        stats_dir = Path(self.target_config["stats_dir"])
        with open(stats_dir / "trace_reconstruction_time.json", "r") as f:
            # time is in millis
            trace_reconstruction_time = float(json.load(f)) / 1_000.0

        # grouped = self.sim_df.groupby(INDEX_COLS, dropna=False, sort=False)
        # print(grouped[PREVIEW_COLS].mean())
        # print(len(grouped))
        self.result_df["trace_time_sec"] = trace_reconstruction_time
