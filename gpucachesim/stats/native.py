...
def stat_cols_for_profiler(profiler: str) -> list[str]:
    stat_cols = [
...
def table_stat_cols_for_profiler(profiler: str) -> list[str]:
    stat_cols = [
        # "num_blocks",
        # "mean_blocks_per_sm",
        # "input_id",
...
class NvprofStats(common.Stats):
    ...
    def compute_native_result_df_nvprof(self):
        ...
        # L1 accesses
        self._compute_l1_reads() # l1 accesses uses l1 reads
        self._compute_l1_accesses()
        self._compute_l1_writes()
    ...
    def _compute_l1_reads(self):
        grouped = self.df.groupby(NVPROF_INDEX_COLS, dropna=False)
        # grouped = self.df.groupby(NVPROF_INDEX_COLS, dropna=False)
        # self.result_df["l1_reads"] = grouped["tex_cache_transactions"].sum()
        self.result_df["l1_reads"] = grouped["tex_cache_transactions"].sum()

    def _compute_l1_accesses(self):
        # grouped = self.df.groupby(NVPROF_INDEX_COLS, dropna=False)
        # note: tex_cache_transaction are only READ transactions
        self.result_df["l1_accesses"] = self.result_df["l1_reads"]
