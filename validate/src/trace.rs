...
use utils::fs::{create_dirs, Bytes, PathExt};
...
pub async fn trace(
    ...
    let output = if let Some((gpu, ref remote)) = remote {
        ...
        if let Some(ref container_image) = trace_options.container_image {
            let hasher = "md5sum";
