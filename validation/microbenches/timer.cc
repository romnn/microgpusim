...
void timer::print_seconds(ostream &str, const char *which, double time) const {

  time = time / CPU_speed_in_MHz;

  time /= (1000 * 1000);
  str << which << " = " << setprecision(6) << setw(9) << time << " seconds";
}
...
ostream &timer::print_simple(ostream &str) {
  //    str << left << setw(25) << (name != 0 ? name : "timer") << ": " <<
  //    right;
