...
CUDAStream<T>::CUDAStream(const int ARRAY_SIZE, const int device_index) {
  ...
  if (props.totalGlobalMem < 3 * ARRAY_SIZE * sizeof(T))
    throw std::runtime_error(
        "Device does not have enough memory for all 3 buffers");
...
template <class T> CUDAStream<T>::~CUDAStream() {
  free(sums);
...
template <class T> void CUDAStream<T>::init_arrays(T initA, T initB, T initC) {
  init_kernel<<<array_size / TBSIZE, TBSIZE>>>(d_a, d_b, d_c, initA, initB,
                                               initC);
  check_error();
...
template <class T>
void CUDAStream<T>::read_arrays(std::vector<T> &a, std::vector<T> &b,
                                std::vector<T> &c) {
...
template <typename T> __global__ void copy_kernel(const T *a, T *c) {
  const int i = blockDim.x * blockIdx.x + threadIdx.x;
  c[i] = a[i];
  // __threadfence_system();
}

template <class T> void CUDAStream<T>::copy() {
...
template <typename T> __global__ void mul_kernel(T *b, const T *c) {
...
template <class T> void CUDAStream<T>::mul() {
...
template <typename T> __global__ void add_kernel(const T *a, const T *b, T *c) {
  const int i = blockDim.x * blockIdx.x + threadIdx.x;
  c[i] = a[i] + b[i];
  // __threadfence_system();
}

template <class T> void CUDAStream<T>::add() {
...
template <class T> void CUDAStream<T>::triad() {
...
template <class T> void CUDAStream<T>::nstream() {
...
template <class T> T CUDAStream<T>::dot() {
...
std::string getDeviceDriver(const int device) {
  ...
  check_error();
  std::ostringstream driver_str;
  driver_str << driver;
  return driver_str.str();
