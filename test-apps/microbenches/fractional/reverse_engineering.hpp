...
/*
 * By what percentage do the bank conflict cause delay as compared to avg read
 * time Used as cutoff
 */
#define GPU_DRAM_OUTLIER_PERCENTAGE 10

/*
 * By what percentage do the cacheline eviction cause delay as compared to avg
 * read time Used as cutoff. Can be high as read from DRAM should be much slower
...
/*
 * For CPU, any values above THRESHOLD_MULTIPLIER * avg is ignored (as might
 * have noise). This is because we are using wall clock time and interrupts
 * might cause huge jumps
 */
