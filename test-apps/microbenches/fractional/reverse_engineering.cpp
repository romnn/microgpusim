...
void print_usage(char **argv) {
  fprintf(
      ...
      argv[0], g_dram_sample_size, g_dram_histogram_spacing);
}
...
bool is_power_of_2(size_t x) { return !(x & (x - 1)); }
...
bool check_dram_partition_pair(void *phy_addr1, void *phy_addr2, void *arg) {
  ...
  data->time =
      device_find_dram_read_time((void *)a, (void *)b, data->threshold);
...
void *find_next_dram_partition_pair(void *phy_addr1, void *phy_start_addr,
                                    void *phy_end_addr, size_t offset,
                                    void *arg) {
  uintptr_t a, b;
...
static double get_histogram_bin(double time, double min_time, int spacing) {
  ...
  return (double)(time_rounded - min_rounded) / (double)spacing;
}

static double get_histogram_bin_start_time(int bin, double min_time,
                                           int spacing) {
  int min_rounded = ((int)(min_time) / spacing) * spacing;
...
static hash_context_t *run_dram_exp(void *virt_start, void *phy_start,
  ...
  if (g_dram_histogram_enabled || g_dram_trendline_enabled) {
    ...
    if (count != g_dram_sample_size) {
      fprintf(stderr,
              "WARNING: DRAM histogram sample size will be %d, instead of %d\n",
  ...
  max = 0;
  sum = 0;
  printf("Finding threshold using sample size of %d\n", count);
  ...
  if (g_dram_histogram_enabled || g_dram_trendline_fp) {
    ...
    printf("Access Time: Threshold is: %f cycles, (Max: %f cycles, Min:%f "
           "cycles)\n",
           running_threshold, max, min);

    /* Create histogram bins - Add few extra bins at end just for verification
     * by eyeballing */
    ...
    if (g_dram_trendline_enabled) {
      print_highlighted(
          "Outputting DRAM access time trendline's raw data to %s",
          g_dram_trendline_file);
      fprintf(g_dram_trendline_fp, "PrimaryAddress\t SecondaryAddress\t "
                                   "AccessTime(cycles)\t Threshold(cycles)\n");
    }
    ...
    if (g_dram_histogram_enabled) {
      ...
      print_highlighted(
          "Outputting DRAM access time histogram's raw data to %s",
          g_dram_histogram_file);
      ...
      for (int i = 0; i < max_bins; i++) {
        ...
        fprintf(g_dram_histogram_fp, "%d-%d\t %d\n", (int)(bin_start_time),
                (int)(bin_start_time) + g_dram_histogram_spacing,
                dram_histogram[i]);
      }
    }
  ...
  phy_end =
      (void *)((uintptr_t)phy_start + allocated - device_allocation_overhead());
  ...
  printf("Physical address bits from which DRAM Banks are possibly derived: "
         "Max Bit:%d, Min Bit:%d\n",
         max_bit, min_bit);
...
void *find_next_cache_partition_pair(void *phy_addr1, void *phy_start_addr,
  ...
  assert((uintptr_t)a == (uintptr_t)phy_addr1);
  assert((uintptr_t)b == (uintptr_t)phy_start_addr);

  dprintf("Trying to find pair for %p in the range [%p, %p)\n", phy_addr1,
          phy_start_addr, phy_end_addr);
  ret_addr = (uintptr_t)device_find_cache_eviction_addr(
      (void *)a, (void *)b, offset, data->running_threshold);
  if (!ret_addr)
    return NULL;

  phy_addr2 =
      (void *)((uintptr_t)ret_addr - data->virt_start + data->phy_start);
  assert((uintptr_t)phy_addr2 == (uintptr_t)ret_addr);
  dprintf("Found valid pair: (%p, %p)\n", phy_addr1, phy_addr2);
...
void *get_next_word(void *start_virt_addr, void *arg) {
  ...
  uintptr_t start_phy_addr =
      (uintptr_t)start_virt_addr - data->virt_start + data->phy_start;
  uintptr_t next_phy_addr;
  bool found = false;

  next_phy_addr = (uintptr_t)hash_get_next_addr(
      data->ctx, data->partition, (void *)start_phy_addr, (void *)-1);
  if (!next_phy_addr)
    return NULL;
 ...
 * memory range.
 */
static hash_context_t *run_cache_exp(void *virt_start, void *phy_start,
                                     size_t allocated, int min_bit,
                                     int max_bit) {
  void *phy_end;
  hash_context_t *hctx;
  ...
  printf("Min bit=%d Max bit=%d PHY range: %08LX to %08LX (%4.2f MiB)\n",
         min_bit, max_bit, (unsigned long long)phy_start,
         (unsigned long long)phy_end,
         (double)((unsigned long long)phy_end - (unsigned long long)phy_start) /
             (1024.0 * 1024.0));
  hctx = hash_init(min_bit, max_bit, phy_start, phy_end);
  assert(hctx);
...
static int run_interference_exp_helper(void *virt_start,
  ...
  ret = device_run_interference_exp(virt_start, get_next_word,
                                    primary_pchase_cb_arg,
                                    secondary_pchase_cb_arg, 50, 1000, time);
  if (ret < 0) {
...
static int run_interference_exp(void *virt_start, void *phy_start,
                                hash_context_t *cache_hctx,
                                hash_context_t *dram_hctx,
  ...
  printf("Running Interference Experiment: Same Cacheline, Same DRAM Bank, "
         "Same Partition\n");
  ...
  printf("Running Interference Experiment: Same Cacheline, Different DRAM "
         "Bank, Same Partition\n");
  ret = run_interference_exp_helper(virt_start, &primary_pchase_cb_arg,
  ...
  printf("Running Interference Experiment: Different Cacheline, Same DRAM "
         "Bank, Same Partition\n");
  ret = run_interference_exp_helper(virt_start, &primary_pchase_cb_arg,
  ...
  printf("Running Interference Experiment: Different Cacheline, Different DRAM "
         "Bank, Same Partition\n");
  ret = run_interference_exp_helper(virt_start, &primary_pchase_cb_arg,
  ...
  print_highlighted(
      "Outputting Interference experiment results' raw data to %s",
      g_interference_file);
  fprintf(g_interference_fp,
          "NumThreads\t SameCachelineSameDRAMBank(SCSB)\t "
          "SameCachelineDifferentDRAMBank(CSDB)\t "
          "DifferentCachelineSameDRAMBank(DCSB)\t "
...
int main(int argc, char *argv[]) {
  ...
  printf("VIRT ADDR START: %08LX\n", (unsigned long long)virt_start);
  printf(" PHY ADDR START: %08LX\n", (unsigned long long)phy_start);

  // printf("Finding DRAM Banks hash function\n");
  // dram_hctx = run_dram_exp(virt_start, phy_start, allocated, min_bit,
  // max_bit); if (dram_hctx == NULL) {
  ...
  cache_hctx =
      run_cache_exp(virt_start, phy_start, allocated, min_bit, max_bit);
  if (cache_hctx == NULL) {
