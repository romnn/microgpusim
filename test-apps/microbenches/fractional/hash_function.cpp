 ...
 */
static int permute_hypothesis(int *array, int size, int min_val, int max_val,
                              int isfirst) {
  int i, start_index;
...
static void add_solution_with_permuations(std::vector<solution_t> &perm_sarray,
                                          solution_t &new_s) {
  int i;
...
static void get_solution_permuations(std::vector<solution_t> &perm_sarray,
                                     solution_t &new_s,
                                     std::vector<solution_t> &output_sarray) {
  int i;
...
/* Returns the full partition number according to the key and a set of
 * hypothesis */
static int get_full_partitions_num(uintptr_t key,
...
static bool
is_solution_correct(std::vector<std::pair<uintptr_t, uintptr_t>> &keys,
                    const solution_t &s) {
  size_t i;
 ...
 */
static int
find_new_solutions(std::vector<std::pair<uintptr_t, uintptr_t>> &keys,
                   int min_bit, int max_bit,
                   std::vector<solution_t> &solutions) {
  int depth;
 ...
 * they need to be.
 */
static bool are_unique_solutions_found(size_t num_solutions, int min_bit,
                                       int max_bit) {
  /*
...
static bool try_find_all_solutions(hash_context_t *ctx) {
  ...
  if (ctx->keys.size() != 0) {
    ...
    if (ctx->solutions.size() > 0) {
      ...
      find_new_solutions(ctx->keys, ctx->min_bit, ctx->max_bit, ctx->solutions);
    }
  }
...
static void hash_confirm_pair(hash_context_t *ctx, uintptr_t phy_addr1,
                              uintptr_t phy_addr2) {
...
static void sort_solutions(std::vector<solution_t> &solutions) {
  ...
  for (s = solutions.begin(); s != solutions.end(); s++) {
    ...
    for (v = variations.begin(), max_index = INT_MIN; v != variations.end();
         v++) {
...
static void get_unique_solutions(std::vector<solution_t> &solutions1,
                                 std::vector<solution_t> &solutions2,
                                 std::vector<solution_t> &out) {
  std::vector<solution_t> perm_sarray;
 ...
 */
hash_context_t *hash_init(int min_bit, int max_bit, void *start_addr,
                          void *end_addr) {
  size_t length;
 ...
 */
static void try_accomodate_new_bit(
    hash_context_t *ctx, int new_bit, void *arg,
    void *(*find_next_partition_pair)(void *addr1, void *start_addr,
                                      void *end_addr, size_t offset,
                                      void *arg)) {
  ...
  for (test_addr = ctx->start_addr + (1ULL << new_bit); test_addr <= end_addr;
       test_addr += offset) {
 ...
 *   Given an address 'addr1', find another address between [start_addr,
 * end_addr] (testing at offset of 'offset') that lies in same partition as
 * 'addr1' and return it. Return NULL if no such pair found.
 *
 * Returns 0 if solution found.
 * Returns < 0 if no solutions found.
 */
int hash_find_solutions(hash_context_t *ctx, void *arg,
                        void *(*find_next_partition_pair)(
                            void *addr1, void *start_addr, void *end_addr,
   ...
   */
  printf(
      "Finding base solutions: min bit=%d max bit=%d start=%08lx end=%08lx\n",
      ctx->min_bit, ctx->max_bit, ctx->start_addr, ctx->end_addr);
  ...
  // end_addr = std::min(ctx->end_addr, end_addr);
  uintptr_t end_addr = ctx->end_addr;
  std::vector<solution_t> new_solutions;
  ...
  printf("config: start bit=%d end bit=%d start=%08lx end=%08lx\n",
         ctx->min_bit, end_bit, ctx->start_addr, end_addr);

  size_t count;
  size_t max_count = (end_addr - ctx->start_addr) / (1ULL << ctx->min_bit) + 1;
  size_t offset = 1ULL << ctx->min_bit;

  printf("=> testing %08lx to %08lx (offset=%lu, limit=%lu) (%4.2f steps)\n",
         ctx->start_addr, end_addr, offset, max_count,
         (double)(end_addr - ctx->start_addr) / (double)offset);
  assert(ctx->start_addr <= end_addr);

  uintptr_t last_addr = LONG_MAX;
  for (test_addr = ctx->start_addr; test_addr <= end_addr;
       test_addr += offset) {

    printf("testing address %08lx ...\n", test_addr);
    assert(base_addr <= test_addr);
    assert(base_addr <= end_addr);
    assert(test_addr < end_addr);
    void *addr = find_next_partition_pair((void *)base_addr, (void *)test_addr,
                                          (void *)end_addr, offset, arg);
    if (addr) {
      ...
      if ((uintptr_t)addr != last_addr) {
        ...
        // avoid infinite loop
        printf("WARN: skipped pair %p to avoid infinite loop\n", addr);
      }
      last_addr = test_addr;
    } else {
      printf("no next partition pair could be found: breaking\n");
      fflush(stdout);
      break;
    }
    ...
    /* Print progress */
    printf("Done: %2.2f percent\n", (float)(count * 100) / (float)(max_count));
    fflush(stdout);
  }
  printf("\n");
  ...
  if (find_new_solutions(ctx->keys, ctx->min_bit, end_bit, ctx->solutions) ==
      0) {
  ...
  if (!are_unique_solutions_found(ctx->solutions.size(), ctx->min_bit,
                                  end_bit)) {
...
int hash_find_solutions2(hash_context_t *ctx, void *arg,
                         bool (*check_partition_pair)(void *addr1, void *addr2,
                                                      void *arg)) {
  cb_arg_t data;
...
hash_context_t *hash_get_common_solutions(hash_context_t *ctx1,
                                          hash_context_t *ctx2) {
...
void *hash_get_next_addr(std::vector<hash_context_t *> ctx,
                         std::vector<int> partition, void *start_addr,
                         void *end_addr) {
  int min_bit = INT_MAX;
  int min_index;
  void *addr;
  ...
  while (!found) {
    addr = get_next_addr(ctx[min_index]->solutions, partition[min_index],
                         prev_addr, end_addr);
...
/* Checks if both address lie on same partition according to set of solutions of
 * ctx */
...
void hash_del(hash_context_t *ctx) { delete ctx; }
