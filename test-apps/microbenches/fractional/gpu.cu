...
// #error \
//     "FGPU_TEST_MEM_COLORING_ENABLED not defined. Needed for reverse
//     engineering"
// #endif
 ...
 */
__device__ uint64_t refresh_l2(volatile uint64_t *refresh_vaddr) {
  uint64_t curindex = 0;
  uint64_t sum = 0;
...
void *device_allocate_contigous(size_t contiguous_size, void **phy_start_p) {
  ...
  // void *base_addr;
  ret = gpuErrCheck(cudaMallocManaged(&gpu_mem, contiguous_size));
  if (ret < 0)
    return NULL;

  // ret = gpuErrCheck(cudaMemPrefetchAsync(g_memory_ctx.base_addr,
  // actual_length, device, stream));
  // ret = gpuErrCheck(cudaMemPrefetch(base_addr, contiguous_size));
  ret = gpuErrCheck(cudaMemPrefetchAsync(gpu_mem, contiguous_size, 0, 0));
...
__global__ void gpu_pchase_setup(uint64_t *array, size_t size, size_t offset) {
  size_t index = blockIdx.x * blockDim.x + threadIdx.x;
  uint64_t num_elem = size / offset;
  if (index < num_elem - 1) {
    array[offset * index / sizeof(uint64_t)] =
        offset * (index + 1) / sizeof(uint64_t);
  }
...
/* Creates a pchase array using base address as 'array' and addresses in
 * 'addresses' */
__global__ void gpu_custom_pchase_setup(uint64_t *array, uint64_t **addresses,
                                        size_t num_entries) {
  size_t index = blockIdx.x * blockDim.x + threadIdx.x;
 ...
 */
void gpu_init_custom_pointer_chase(void *base_addr,
                                   void *(*cb)(void *addr, void *arg),
                                   void *arg, int num_entries,
  ...
  gpuErrAssert(cudaMemcpy(d_custom_pchase, h_custom_pchase,
                          sizeof(uint64_t *) * found, cudaMemcpyHostToDevice));

  assert(found > 0);

  num_blocks = (found + num_threads - 1) / num_threads;
  gpu_custom_pchase_setup<<<num_blocks, num_threads>>>(h_custom_pchase[0],
                                                       d_custom_pchase, found);
  gpuErrAssert(cudaDeviceSynchronize());
...
__global__ void gpu_modify_pointer_chase(uint64_t *ct_min_addr,
                                         uint64_t index) {
  *ct_min_addr = index;
}
...
int device_init(size_t req_reserved_size, size_t *reserved_size) {
  ...
  resv_memory = std::min(req_reserved_size, max_len);
  ...
  gpuErrAssert(cudaMalloc(&d_custom_pchase,
                          max_custom_pchase_entires * sizeof(uint64_t *)));
  h_custom_pchase =
      (uint64_t **)malloc(max_custom_pchase_entires * sizeof(uint64_t *));
  ...
  gpuErrCheck(
      cudaMemcpy(d_refresh_v, h_refresh_v, l2_size, cudaMemcpyHostToDevice));
...
size_t device_allocation_overhead(void) { return 2 * GPU_L2_CACHE_LINE_SIZE; }
 ...
 */
__global__ void read_dram_pair(volatile uint64_t *a_v, volatile uint64_t *b_v,
                               volatile uint64_t *refresh_v,
                               volatile double *ticks, volatile uint64_t *psum,
                               double threshold) {
  uint64_t curindex;
  uint64_t sum;
  uint64_t count;
...
double device_find_dram_read_time(void *_a, void *_b, double threshold) {
  ...
  gpuErrAssert(cudaDeviceSynchronize());
  gpuErrAssert(cudaMemcpy(h_ticks, d_ticks, GPU_MAX_OUTER_LOOP * sizeof(double),
                          cudaMemcpyDeviceToHost));

  // for (i = 0, min_ticks = (double)LONG_MAX, sum_ticks = 0, max_ticks = 0;
  for (i = 0, min_ticks = DBL_MAX, sum_ticks = 0, max_ticks = 0;
  ...
  dprintf("Min Ticks: %0.3f,\tAvg Ticks: %0.3f,\tMax Ticks: %0.3f\n", min_ticks,
          (sum_ticks * 1.0f) / GPU_MAX_OUTER_LOOP, max_ticks);
...
is_cacheline_evicted(volatile uint64_t *base, uint64_t count,
                     uint64_t threshold, bool *reached_end, uint64_t **ret_addr,
 ...
 * Psum is just to avoid compiler optimizations.
 */
__global__ void evict_cacheline(volatile uint64_t *base, uint64_t start_count,
                                uint64_t threshold, uint64_t **ret_addr,
                                uint64_t *ret_count, volatile uint64_t *psum) {
...
#pragma unroll 1
  while (is_cacheline_evicted(base, count, threshold, &reached_end, &addr,
                              &local_sum) == false &&
         reached_end == false) {
  ...
  while (lower_bound < upper_bound) {
    ...
    if (is_cacheline_evicted(base, count, threshold, &reached_end, &addr,
                             &local_sum) == false) {
...
success:
  /* Just checks */
  if (false) {
    // these fail for the virtual addresses on my active GTX1080
 ...
 */
__global__ void cacheline_read_time(volatile uint64_t *base,
                                    volatile uint64_t *end_addr, double *ticks,
                                    volatile uint64_t *psum) {
  uint64_t curindex;
  volatile uint64_t *lastaddr;
  uint64_t count;
  ...
  for (sum = 0, count = 0; count < GPU_MAX_OUTER_LOOP + 1; count++) {
    ...
    lastaddr = base;
    /* Read bunch of words */
    for (int i = 0; i < count && ((uintptr_t)lastaddr < (uintptr_t)end_addr);
         i++) {
      lastaddr = &base[curindex];
      curindex = *lastaddr;
      sum += curindex;
...
int device_cacheline_test_find_threshold(size_t sample_size, double *avg) {
  ...
  for (i = 0; i < sample_size; i++) {
    cacheline_read_time<<<1, 1>>>((uint64_t *)ct_start_addr,
                                  (uint64_t *)ct_end_addr, d_ticks, d_sum);
    gpuErrAssert(cudaDeviceSynchronize());
    gpuErrAssert(cudaMemcpy(h_ticks, d_ticks,
                            GPU_MAX_OUTER_LOOP * sizeof(double),
                            cudaMemcpyDeviceToHost));

    // for (j = 0, min_ticks = LONG_MAX, sum_ticks = 0, max_ticks = 0;
    for (j = 0, min_ticks = DBL_MAX, sum_ticks = 0, max_ticks = 0;
 ...
 */
void *device_find_cache_eviction_addr(void *_a, void *_b, size_t offset,
                                      double threshold) {
  ...
  if (b != ct_last_start_addr && b != ct_last_start_addr + ct_offset) {
    ...
    /* Round up b */
    b = ct_start_addr +
        (((b - ct_start_addr) + ct_offset - 1) / ct_offset) * ct_offset;
     ...
     */
    gpu_modify_pointer_chase<<<1, 1>>>((uint64_t *)ct_start_addr + ct_offset,
                                       index);
    gpuErrAssert(cudaDeviceSynchronize());
  ...
  gpuErrAssert(cudaDeviceSynchronize());
  gpuErrAssert(cudaMemcpy(&last_addr, d_last_addr, sizeof(uint64_t *),
                          cudaMemcpyDeviceToHost));
  gpuErrAssert(cudaMemcpy(&ct_start_count, d_count, sizeof(uint64_t *),
                          cudaMemcpyDeviceToHost));

  /* Edit p-chase so as to skip last address next time */
  index = ((uintptr_t)last_addr + ct_offset - ct_start_addr) / sizeof(uint64_t);
  gpu_modify_pointer_chase<<<1, 1>>>(
      (uint64_t *)((uintptr_t)last_addr - ct_offset), index);
  gpuErrAssert(cudaDeviceSynchronize());
...
/*
 * Finds the number of words in a cacheline. Requires that device must be
 * initilized with cacheline specific pchase.
 */
int device_find_cacheline_words_count(void *gpu_start_addr, double threshold,
                                      void *(*cb)(void *addr, void *arg),
                                      void *arg, size_t *words) {
  uint64_t count;
  int ret;
  void *start_addr;

  ret = device_custom_pchase_init(gpu_start_addr, cb, arg, INT_MAX, &start_addr,
                                  NULL);
  if (ret < 0)
    return ret;
 ...
 * on other sms.
 */
__global__ void do_interference_exp(volatile uint64_t **bases,
                                    int num_interferening_blocks,
                                    int *sm0_blocks_count,
                                    int *interfereing_blocks_count,
                                    int loop_count, double *out_ticks,
                                    volatile uint64_t *psum) {
...
#pragma unroll 1
  ...
  if (blockIdx.x == 0)
    dprintf("Interference Exp: Block:%d, Loop Count:%d, Num Blocks:%d, "
            ...
            blockIdx.x, loop_count, gridDim.x, (double)avg_ticks,
            (double)min_ticks, (double)max_ticks);
 ...
 * interfering SMs.
 */
int device_run_interference_exp(void *gpu_start_addr,
                                void *(*cb)(void *addr, void *arg),
                                void *primary_arg, void *secondary_arg,
                                int max_blocks, int loop_count,
  ...
  gpuErrAssert(cudaMemcpy(d_base_address, h_base_address,
                          sizeof(uint64_t *) * max_blocks,
  ...
  for (int i = 1; i <= max_blocks; i++) {
    ...
    /* Launch some extra blocks, just in case. We deal with extra blocks inside
     * kernel */
    do_interference_exp<<<2 * max_blocks, 1>>>(
        (volatile uint64_t **)d_base_address, i - 1, d_sm0_blocks,
        d_inteferening_blocks, loop_count, d_ticks, d_sum);
    gpuErrAssert(cudaDeviceSynchronize());
    gpuErrAssert(
        cudaMemcpy(&ticks, d_ticks, sizeof(double), cudaMemcpyDeviceToHost));
