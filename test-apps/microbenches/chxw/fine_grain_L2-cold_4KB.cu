...
void parametric_measure_global(int N, int iterations, int stride) {
  ...
  unsigned int *h_timeinfo =
      (unsigned int *)malloc(sizeof(unsigned int) * 4096);

  unsigned int *duration;
  cudaMalloc((void **)&duration, sizeof(unsigned int) * 4096);

  unsigned int *d_index;
  cudaMalloc((void **)&d_index, sizeof(unsigned int) * 4096);
  ...
  cudaMemcpy((void *)h_timeinfo, (void *)duration, sizeof(unsigned int) * 4096,
             cudaMemcpyDeviceToHost);
  cudaMemcpy((void *)h_index, (void *)d_index, sizeof(unsigned int) * 4096,
             cudaMemcpyDeviceToHost);
