...
def doProcess(src: str, dst: str, direction="auto"):
    ...
    if direction == "bin2asm" or fext in {".cubin", ".bin"}:
        ...
        print(
            'The first infile should be with ext ".cubin" or ".cuasm", otherwise specify direction by option --bin2asm or --asm2bin!'
        )
        exit(-1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Input filename. If not with extension .cubin/.bin/.cuasm/.asm, direction option --bin2asm or --asm2bin should be specified.",
    )
    parser.add_argument(
        "-o",
        "--output",
        dest="outfile",
        help="Output filename, inferred from input filename if not given.",
    )
    parser.add_argument(
        "-f",
        "--logfile",
        dest="logfile",
        help="File name for saving the log, default to none.",
    )
    ...
    group_loglevel.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="Verbose mode, showing almost every log.",
    )
    group_loglevel.add_argument(
        "-q",
        "--quiet",
        action="store_true",
        help="Quiet mode, no log unless errores found.",
    )
