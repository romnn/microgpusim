...
class CubinFile:
    ...
    # def __init__(self, cubin_bytes, name=None):
    #     test = BytesIO(bytes(cubin_bytes))
    #     test.seek(0)
    #     magic = test.read(4)
    #     assert magic == b"\x7fELF"
    #     self.__mCubinBytes = bytes(cubin_bytes)
    #
        ...
        for iseg, segh in enumerate(self.__mELFSegments):
            if segh["p_type"] == "PT_LOAD":  # only P_LOAD type needs range
                ...
                if p0 not in sec_start_dict:
                    ...
                    max_d = max(
                        [k if k < p0 else -10 for k in sec_start_dict]
                    )  # find last section start before current bound
                    if max_d == -10:
                ...
                if p1 not in sec_end_dict:
                    ...
                    min_d = min(
                        [k if k > p1 else 2**32 for k in sec_end_dict]
                    )  # find first section end after current bound
                    if min_d == 2**32:
    ...
    def __writeFileHeaderAsm(self, stream, ident="\t"):
        ...
        stream.write(
            ident
            + ".__elf_phoff            0x%x \t\t// program header offset, maybe updated by assembler\n"
            % fheader["e_phoff"]
        )
        stream.write(
            ident
            + ".__elf_shoff            0x%x \t\t// section header offset, maybe updated by assembler\n"
            % fheader["e_shoff"]
        )
        ...
        stream.write(
            ident
            + ".__elf_flags            0x%x \t\t// Flags, SM_%d(0x%x), COMPUTE_%d(0x%x) \n"
        ...
        stream.write(
            ident
            + ".__elf_shnum            %d \t\t// number of sections, currently no sections can be appended/removed\n"
            % fheader["e_shnum"]
        )
        stream.write(
            ident + ".__elf_shstrndx         %d \t\t// Section name string table index \n" % fheader["e_shstrndx"]
        )
        stream.write("\n")
    ...
    def __writeImplicitSectionAsm(self, stream, secname):
        ...
        if secname == ".shstrtab" or secname == ".strtab":
            ...
            for sym in sym_section.iter_symbols():
                ...
                stream.write(
                    bytes2Asm(
                        data[isym * sym_entsize : (isym + 1) * sym_entsize],
                        addr_offset=isym * sym_entsize,
                    )
                )
                stream.write("\n")
                isym += 1
    ...
    def __writeSegmentHeaderAsm(self, stream, segheader, segrange):
        ...
        stream.write(
            "  .__segment_filesz  0x%x   \t\t// file size, maybe updated by assembler \n" % (segheader["p_filesz"])
        )
        stream.write(
            "  .__segment_memsz   0x%x   \t\t// file size + nobits sections, maybe updated by assembler \n"
            % (segheader["p_memsz"])
        )
